...
use super::{SimpleEmulator, FrontendResult};
...
pub fn run_cli() -> FrontendResult<()> {
    ...
    match &args.command {
        Some(Commands::Run { rom_file, cycles, debug }) => {
            run_rom_command(rom_file, *cycles, *debug || args.debug)
        }
        Some(Commands::Info { rom_file }) => {
            show_rom_info(rom_file)
        }
        Some(Commands::Validate { rom_file }) => {
            validate_rom(rom_file)
        }
        Some(Commands::List { item }) => {
            list_items(item)
        }
        ...
        None => {
            ...
            if let Some(rom_file) = &args.rom_file {
                run_rom_command(rom_file, 1000, args.debug)
            } else {
...
/// Runs a ROM file with the emulator.
fn run_rom_command(rom_file: &PathBuf, cycles: u32, debug: bool) -> FrontendResult<()> {
        ...
        match emulator.step() {
            Ok(()) => {
                ...
                if curr_state.sound_timer > prev_state.sound_timer {
                    ...
                    if !debug { log::info!("🔊 Sound timer set to {} at cycle {}", curr_state.sound_timer, i + 1); }
                }
                if pc_diff != 2 {
                    jump_instructions += 1;
                    if !debug && jump_instructions <= 3 { 
                        log::info!("🔄 Jump/Call instruction at cycle {}: PC 0x{:04X} -> 0x{:04X}", 
                               i + 1, prev_state.pc, curr_state.pc); 
                    }
                }
                
                if debug && i < 10 {
                    log::debug!("Cycle {}: PC=0x{:04X}, I=0x{:04X}, SP={}", 
                        i + 1, curr_state.pc, curr_state.i, curr_state.sp);
                }
                
                // Show progress every 100 cycles
                if !debug && i % 100 == 0 && i > 0 {
                    println!("⏱️  Progress: {}/{} cycles ({:.1}%)", i, cycles, (i as f32 / cycles as f32) * 100.0);
                }
            }
            Err(e) => {
    ...
    if final_state.sound_timer > 0 {
        println!("   🎵 Sound timer active: {} frames remaining", final_state.sound_timer);
    }
    if final_state.delay_timer > 0 {
        println!("   ⏰ Delay timer active: {} frames remaining", final_state.delay_timer);
    }
    
    // Check if any registers were modified
    let modified_regs: Vec<usize> = final_state.v.iter()
        .enumerate()
        .filter(|(_, &val)| val != 0)
        .map(|(i, _)| i)
        .collect();
    ...
    if !modified_regs.is_empty() {
        ...
        for &reg in &modified_regs {
            println!("      V{:X} = 0x{:02X} ({})", reg, final_state.v[reg], final_state.v[reg]);
        }
    }
    ...
    println!("💡 Try: cargo run -- info {} for ROM analysis", rom_file.display());
