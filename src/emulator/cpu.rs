...
use crate::hardware::{Display, Audio, Input, ChipKey};
...
impl Cpu {
    ...
    pub fn load_rom(&mut self, rom_data: &[u8]) -> Result<()> {
        ...
        self.registers.set_pc(crate::emulator::memory::PROGRAM_START);
        Ok(())
    }
    ...
    fn execute_instruction(&mut self, instruction: u16) -> Result<()> {
        ...
        match nibbles {
            ...
            (0x2, _, _, _) => {
                log::debug!("CALL {:#04x} (PC before push: {:#04x}, SP: {})", nnn, self.registers.get_pc(), self.stack.depth());
                self.call(nnn)?;
            ...
            (0xF, _, 0x6, 0x5) => {
                ...
                self.ld_vx_i(x)?;
                log::debug!("V0-V{:X} after LD: {:?}", x, self.registers.get_v_range(0, x as usize + 1)?);
                log::debug!("I after LD: {:#04x}", self.registers.get_i());
            },
            
            _ => return Err(EmulatorError::UnknownInstruction { opcode: instruction }),
        }
    ...
    fn drw(&mut self, x: u8, y: u8, n: u8) -> Result<()> {
        ...
        if let Some(display) = &mut self.display {
            ...
            for i in 0..n {
                ...
                if addr < 4096 { // MEMORY_SIZE
                    sprite_data.push(self.memory.read_byte(addr as u16)?);
                }
            }
            
            // Draw sprite and check for collision
            collision = display.draw_sprite(x_pos as u8, y_pos as u8, &sprite_data).unwrap_or(false);
        }
    ...
    fn skp_vx(&mut self, x: u8) -> Result<()> {
        ...
        if let Some(input) = &self.input {
            if let Some(chip_key) = ChipKey::from_u8(key_value) {
                ...
                log::debug!("SKP V{:X} (key {:#X}): is_pressed = {}", x, key_value, is_pressed);
                if is_pressed {
    ...
    fn sknp_vx(&mut self, x: u8) -> Result<()> {
        ...
        if let Some(input) = &self.input {
            if let Some(chip_key) = ChipKey::from_u8(key_value) {
                let is_pressed = input.borrow().is_key_pressed(chip_key);
                log::debug!("SKNP V{:X} (key {:#X}): is_pressed = {}", x, key_value, is_pressed);
                if !is_pressed {
                    self.registers.set_pc(self.registers.get_pc() + 2);
                }
            }
...
mod tests {
    ...
    fn test_key_wait() {
        use std::rc::Rc;
        use std::cell::RefCell;
        use crate::hardware::input::{SoftwareInput, ChipKey};
    ...
    fn test_unknown_instruction() {
        ...
        assert!(matches!(result, Err(EmulatorError::UnknownInstruction { .. })));
    }
