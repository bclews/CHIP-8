...
use cpal::{
    ...
    BufferSize, Device, SampleFormat, SampleRate, Stream, StreamConfig as CpalStreamConfig,
};
use std::sync::{Arc, Mutex};

use super::{
    buzzer::{AudioBuzzer, BuzzerState},
    AudioSystemResult,
};
...
impl StreamConfig {
    ...
    pub fn low_latency() -> Self {
        Self::new().with_buffer_size(256).with_sample_rate(48000)
    }
...
mod tests {
    ...
    fn test_sample_format_conversion() {
        assert_eq!(
            SampleFormat::from(StreamSampleFormat::F32),
            SampleFormat::F32
        );
        assert_eq!(
            SampleFormat::from(StreamSampleFormat::I16),
            SampleFormat::I16
        );
        assert_eq!(
            SampleFormat::from(StreamSampleFormat::U16),
            SampleFormat::U16
        );
    }
