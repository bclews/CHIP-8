...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert_eq!(
            error.to_string(),
            "Invalid memory access at address 0x1234"
        );
    }

    #[test]
    fn test_rom_too_large_error() {
        let error = EmulatorError::RomTooLarge { size: 5000, max_size: 3584 };
        assert_eq!(
            error.to_string(),
    ...
    fn test_audio_error_conversion() {
        ...
        assert!(matches!(emulator_err, EmulatorError::AudioError(AudioError::InitializationFailed)));
    }
    ...
    fn test_graphics_error_conversion() {
        ...
        assert!(matches!(emulator_err, EmulatorError::Graphics(GraphicsError::InvalidCoordinates { x: 100, y: 200 })));
    }
}